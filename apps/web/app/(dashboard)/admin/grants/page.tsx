    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 grid gap-3">
        <input
          ...
          className="border border-slate-200 rounded-lg px-3 py-2 text-slate-900 placeholder:text-slate-400"
        />
        <input
          value={deptIds}
          onChange={e => setDeptIds(e.target.value)}
          placeholder="Department UUIDs, comma separated"
          className="border border-slate-200 rounded-lg px-3 py-2 text-slate-900 placeholder:text-slate-400"
        />
        <button onClick={grant} className="rounded-lg bg-blue-600 text-white px-4 py-2 text-sm w-fit">
          Create Grant
        </button>
      </div>
